...
function _setup_trim_env()
    ...
    # MySQL from the local checkout and adds JuliaC.
    ...
    Pkg.add(["JuliaC", "DBInterface", "Tables", "Dates"])
    """)
    ...
    mktempdir() do tmpdir
        cd(tmpdir) do
            ...
            if trim_errors == 0
                ...
                @test !run_timed_out
                @test run_exit == 0
                @test occursin("mysql trim workload passed", run_output)
            else
                @test exit_code != 0
            end
        end
...
@testset "Trim compile" begin
    if !Base.get_bool_env("MYSQL_RUN_TRIM_TESTS", true)
        println("[trim] skip MYSQL_RUN_TRIM_TESTS=false: user requested to skip trim compilation tests")
        @test true
