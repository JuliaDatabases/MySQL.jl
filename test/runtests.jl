...
# 186
DBInterface.execute(conn, "DROP TABLE if exists zero_dates")
DBInterface.execute(conn, "CREATE TABLE zero_dates (dt TIMESTAMP)")
stmt = DBInterface.prepare(conn, "INSERT INTO zero_dates (dt) VALUES (?);")
DBInterface.execute(stmt, ("0000-00-00 00:00:00",))
res = DBInterface.execute(conn, "select dt from zero_dates") |> columntable
@test length(res) == 1
@test res[1][1] === Int32(-1)