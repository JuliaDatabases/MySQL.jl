...
function parse_column_def(p::PacketView; extended_metadata::Bool=false)
    ...
    for (field, value) in (("catalog", catalog), ("schema", schema), ("table", table), ("org_table", org_table), ("name", name), ("org_name", org_name))
        isvalid(value) || protocol_error("malformed column definition: $field is not valid UTF-8")
    end
    ...
    length = read_u32!(c)
    type = read_u8!(c)
    type == MYSQL_TYPE_VECTOR && protocol_error("MYSQL_TYPE_VECTOR columns are not supported")
    flags = read_u16!(c)
    decimals = read_u8!(c)
